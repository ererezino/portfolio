...
class SwipeableGallery {
  ...
  render() {
    ...
    hint.innerHTML = `<span>Swipe or click</span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>`;
  ...
  bindEvents() {
    this.cards.forEach(card => {
      // Click to open lightbox or cycle
...
function renderContent() {
  ...
  if (photoGrid) {
    ...
    photoGrid.querySelectorAll('.photo-item').forEach(el => {
      el.addEventListener('click', () => openLightbox(parseInt(el.dataset.photoIndex), el));
    });
  }
    ...
    articleGrid.innerHTML = ARTICLES.map(a => `
      <a class="article-card" href="${a.url}">
        ...
        <div class="article-card-bottom">
          ...
          <div class="article-card-arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 17L17 7M17 7H7M17 7v10"/></svg></div>
        </div>
      </a>
    `).join('');
  }
  ...
  if (travelGrid) {
    travelGrid.innerHTML = TRAVELS.map(t => `
      <a class="travel-card" href="${t.url}">
        ...
        <div class="travel-card-arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></div>
      </a>
    `).join('');
  }
  ...
  document.getElementById('musicList').innerHTML = MUSIC.map(m => `
    <a href="${m.url}" target="_blank" rel="noopener noreferrer" class="into-item">
      <div class="into-cover"><img src="${m.cover}" alt="${m.title}" loading="lazy"></div>
      <div class="into-text"><div class="into-title">${m.title}</div><div class="into-meta">${m.artist}</div></div>
    </a>
  `).join('');

  document.getElementById('bookList').innerHTML = BOOKS.map(b => `
    <a href="${b.url}" target="_blank" rel="noopener noreferrer" class="into-item">
      <div class="into-cover book"><img src="${b.cover}" alt="${b.title}" loading="lazy"></div>
      <div class="into-text"><div class="into-title">${b.title}</div><div class="into-meta">${b.author}</div></div>
    </a>
  `).join('');

  document.getElementById('tvList').innerHTML = TV.map(t => `
    <a href="${t.url}" target="_blank" rel="noopener noreferrer" class="into-item">
      <div class="into-cover"><img src="${t.cover}" alt="${t.title}" loading="lazy"></div>
      <div class="into-text"><div class="into-title">${t.title}</div><div class="into-meta">${t.year}</div></div>
    </a>
  `).join('');
      ...
      return `
        <div class="month-card ${stateClass}">
          ...
          <div class="progress-wrapper">
            <div class="progress-track"><div class="progress-fill" data-width="${barWidth}"></div></div>
          </div>
          <div class="month-details">
            <div><div class="month-detail-value">${m.km} km</div><div class="month-detail-label">Distance</div></div>
            <div><div class="month-detail-value">${fmt(m.avgStepsPerDay)}</div><div class="month-detail-label">Daily Avg</div></div>
          </div>
        </div>
      `;
    }).join('');
